...
public class RTPService implements RandomTeleportService {
	...
	public RTPService(CommandPackInstance plugin) {
		this.plugin = plugin;
	}
	...
	public Optional<ServerLocation> getLocation(ServerLocation currentLocation, RandomTeleportOptions options) {
		...
		while(attempts < options.getAttempts()) {
			...
			if(options.getProhibitedBiomes().isEmpty() || !options.getProhibitedBiomes().contains(Biomes.registry().valueKey(world.biome(newPos)).asString())) {
	...
	private Optional<Integer> getRandomX(ServerLocation currentLocation, ServerWorld world, RandomTeleportOptions options, int attempts) {
		...
		int borderRadius = (int) (world.border().diameter() / 2);
		if(x > world.border().center().floorX() - borderRadius && x < world.border().center().floorX() + borderRadius) {
			return Optional.ofNullable(x);
		}
		return getRandomX(currentLocation, world, options, attempts);
	}
	...
	private Optional<Integer> getRandomZ(ServerLocation currentLocation, ServerWorld world, RandomTeleportOptions options, int attempts) {
		...
		int borderRadius = (int) (world.border().diameter() / 2);
		if(z > world.border().center().floorY() - borderRadius && z < world.border().center().floorY() + borderRadius) {
			return Optional.ofNullable(z);
		}
		return getRandomZ(currentLocation, world, options, attempts);
	}
