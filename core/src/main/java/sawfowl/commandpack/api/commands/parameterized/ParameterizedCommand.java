...
public interface ParameterizedCommand extends PluginCommand, CommandExecutor {
	...
	default CommandResult execute(CommandContext context) throws CommandException {
		...
		if(!isPlayer && onlyPlayer()) exception(CommandPackInstance.getInstance().getLocales().getAsReferenced(getLocale(context.cause())).getCommandExceptions().getOnlyPlayer());
		Locale locale = getLocale(context.cause());
		if(!isPlayer && onlyPlayer()) exception(CommandPackInstance.getInstance().getLocales().getAsReferenced(locale).getCommandExceptions().getOnlyPlayer());
		...
		execute(context, context.cause().first(ServerPlayer.class).map(player -> (Audience) player).orElse(context.cause().root() instanceof Audience audience ? audience : context.cause().audience()), locale, isPlayer);
		return success();
	}
