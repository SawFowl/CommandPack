...
public class PlayerChatListener {
	...
	public void onSendMessage(PlayerChatEvent.Submit event, @First ServerPlayer player) {
		...
		player.sendMessage(plugin.getLocales().getAsReferenced(player.locale()).getCommands().getMute().getSuccessTarget(mute.getExpiration().isPresent(), mute.getSource().orElse(text("&e-")), expire(player.locale(), mute), mute.getReason().orElse(text("&e-"))));
	}
