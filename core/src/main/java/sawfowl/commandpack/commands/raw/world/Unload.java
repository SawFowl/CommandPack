...
public class Unload extends AbstractWorldCommand {
	...
	public Unload(CommandPackInstance plugin) {
		super(plugin);
	}
	...
	private RawArgument<ServerWorld> createWorldArgument() {
		return RawArgument.of(
			ServerWorld.class,
			(_, _) -> Sponge.server().worldManager().worlds().stream().filter(w -> w.isLoaded() && !w.key().asString().equals(DefaultWorldKeys.DEFAULT.asString())).map(w -> w.key().asString()),
			(_, args) -> args.length >= 1 ? Sponge.server().worldManager().worlds().stream().filter(w -> w.isLoaded() && !w.key().asString().equals(DefaultWorldKeys.DEFAULT.asString()) && w.key().asString().equals(args[0])).findFirst() : Optional.empty(),
