...
public class SetWarp extends AbstractPlayerCommand {
	...
	public void execute(CommandContext context, ServerPlayer src, Locale locale) throws CommandException {
		delay(src, locale, _ -> {
			...
			if(warpIsPresent(name, admin, src)) {
				if(admin) exception(plugin.getLocales().getAsReferenced(locale).getCommands().getSetWarp().getAllreadyExist());
			...
			if(admin) {
				...
				src.sendMessage(plugin.getLocales().getAsReferenced(locale).getCommands().getSetWarp().getSuccessAdmin());
				...
				src.sendMessage(plugin.getLocales().getAsReferenced(locale).getCommands().getSetWarp().getSuccess());
			} else exception(plugin.getLocales().getAsReferenced(locale).getCommands().getSetWarp().getLimit(Permissions.getWarpsLimit(src)));
		});
	}
	...
	public List<ParameterSettings> getParameterSettings() {
		return Arrays.asList(
			ParameterSettings.of(CommandParameters.createString("Warp", false), false, locale -> plugin.getLocales().getAsReferenced(locale).getCommandExceptions().getNameNotPresent()),
			ParameterSettings.of(CommandParameters.createBoolean("Private", false), false, locale -> plugin.getLocales().getAsReferenced(locale).getCommandExceptions().getBooleanNotPresent()),
			ParameterSettings.of(CommandParameters.createBoolean("Admin", Permissions.WARP_STAFF, true), true, locale -> plugin.getLocales().getAsReferenced(locale).getCommandExceptions().getBooleanNotPresent())
		);
	}
