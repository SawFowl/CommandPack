...
public class Day extends AbstractParameterizedCommand {
	...
	public Day(CommandPackInstance plugin) {
		super(plugin);
	}
	...
	private void setTime(Audience src, Locale locale, ServerWorld world) {
		CPServerWorld.cast(world).getWorldTime().setDay();
		...
		src.sendMessage(plugin.getLocales().getAsReferenced(locale).getCommands().getTime().getDay(world));
	}
