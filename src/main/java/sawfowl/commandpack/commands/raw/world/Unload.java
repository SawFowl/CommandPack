...
public class Unload extends AbstractWorldCommand {
	...
	public void process(CommandCause cause, Audience audience, Locale locale, boolean isPlayer, String[] args, Mutable arguments) throws CommandException {
		...
		if(!Sponge.server().worldManager().world(ResourceKey.resolve(args[0])).get().isLoaded()) exceptionAppendUsage(cause, TextUtils.replace(getText(locale, LocalesPaths.COMMANDS_WORLD_UNLOADED), Placeholders.WORLD, args[0]));
	...
	public List<CommandCompletion> complete(CommandCause cause, List<String> args, Mutable arguments, String currentInput) throws CommandException {
		...
		if(args.size() == 0) return Sponge.server().worldManager().worlds().stream().filter(ServerWorld::isLoaded).map(ServerWorld::key).map(ResourceKey::asString).filter(k -> !k.equals(DefaultWorldKeys.DEFAULT.asString())).map(CommandCompletion::of).collect(Collectors.toList());
		if(args.size() == 1 && !currentInput.endsWith(" ")) return Sponge.server().worldManager().worlds().stream().filter(ServerWorld::isLoaded).map(ServerWorld::key).map(ResourceKey::asString).filter(k -> (!k.equals(DefaultWorldKeys.DEFAULT.asString()) && (k.split(":")[1].startsWith(args.get(0))) || (args.get(0).contains(k) && !args.get(0).contains(k + " ")))).map(CommandCompletion::of).collect(Collectors.toList());
	...
	public Component usage(CommandCause cause) {
		return text("&c/cworld unload <World>");
	}
