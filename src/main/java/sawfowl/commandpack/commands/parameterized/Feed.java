...
public class Feed extends AbstractParameterizedCommand {
	...
	public Feed(CommandPackPlugin plugin) {
		super(plugin);
	}
	...
	public void execute(CommandContext context, Audience src, Locale locale, boolean isPlayer) throws CommandException {
		if(isPlayer) {
			...
			if(target.isPresent() && !target.get().uniqueId().equals(((ServerPlayer) src).uniqueId())) {
				...
				src.sendMessage(TextUtils.replace(getText(locale, LocalesPaths.COMMANDS_FEED_OTHER), Placeholders.PLAYER, target.get().name()));
				target.get().sendMessage(getText(target.get(), LocalesPaths.COMMANDS_FEED_SELF));
			} else {
				((ServerPlayer) src).offer(Keys.FOOD_LEVEL, ((ServerPlayer) src).getOrElse(Keys.MAX_FOOD_LEVEL, 20));
				src.sendMessage(getText(locale, LocalesPaths.COMMANDS_FEED_SELF));
			}
		} else {
			ServerPlayer player = getPlayer(context).get();
			player.offer(Keys.FOOD_LEVEL, player.getOrElse(Keys.MAX_FOOD_LEVEL, 20));
			src.sendMessage(TextUtils.replace(getText(locale, LocalesPaths.COMMANDS_FEED_OTHER), Placeholders.PLAYER, player.name()));
			player.sendMessage(getText(player, LocalesPaths.COMMANDS_FEED_SELF));
		}
	}
