...
public class Warn extends AbstractParameterizedCommand {
	...
	private void tryMute(User user, Warns warns, Audience src, Locale locale) {
		...
		if(plugin.getMainConfig().getPunishment().getAnnounce().isBan()) {
			if(mute.getExpirationDate().isPresent()) {
				...
				Sponge.server().onlinePlayers().forEach(player -> {
					player.sendMessage(TextUtils.replaceToComponents(getText(player, LocalesPaths.COMMANDS_MUTE_ANNOUNCEMENT_PERMANENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {getText(player, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), user.get(Keys.DISPLAY_NAME).orElse(text(user.name())), getText(player, LocalesPaths.COMMANDS_WARN_MUTE_LIMIT)}));
				});
			}
		} else {
	...
	private void tryKick(ServerPlayer target, Audience src, Locale locale) {
		target.kick(TextUtils.replaceToComponents(getText(target, LocalesPaths.COMMANDS_KICK_DISCONNECT), new String[] {Placeholders.SOURCE, Placeholders.VALUE}, new Component[] {getText(target, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), TextUtils.replace(getText(target, LocalesPaths.COMMANDS_WARN_KICK_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getKick()))}));
		Sponge.systemSubject().sendMessage(TextUtils.replaceToComponents(getText(plugin.getLocales().getLocaleService().getDefaultLocale(), LocalesPaths.COMMANDS_KICK_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {getText(target, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), text(target.name()), TextUtils.replace(getText(plugin.getLocales().getLocaleService().getSystemOrDefaultLocale(), LocalesPaths.COMMANDS_WARN_KICK_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getKick()))}));
		if(plugin.getMainConfig().getPunishment().getAnnounce().isKick()) {
			...
			Sponge.server().onlinePlayers().forEach(player -> {
				player.sendMessage(TextUtils.replaceToComponents(getText(player, LocalesPaths.COMMANDS_KICK_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {getText(target, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), targetName, TextUtils.replace(getText(player, LocalesPaths.COMMANDS_WARN_KICK_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getKick()))}));
			});
	...
	private void tryBan(User user, Warns warns, Audience src, Locale locale, Component source) {
		...
		if(user.player().isPresent() && user.isOnline()) user.player().get().kick(TextUtils.replaceToComponents(getText(user.player().get(), ban.expirationDate().isPresent() ? LocalesPaths.COMMANDS_BAN_DISCONNECT : LocalesPaths.COMMANDS_BAN_DISCONNECT_PERMANENT), new String[] {Placeholders.TIME, Placeholders.SOURCE, Placeholders.VALUE}, new Component[] {(ban.expirationDate().isPresent() ? expire(user.player().get().locale(), ban) : text("")), getText(user.player().get(), LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), TextUtils.replace(getText(user.player().get(), LocalesPaths.COMMANDS_WARN_BAN_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getBan()))}));
		if(ban.expirationDate().isPresent()) {
			Sponge.systemSubject().sendMessage(TextUtils.replaceToComponents(getText(plugin.getLocales().getLocaleService().getSystemOrDefaultLocale(), LocalesPaths.COMMANDS_BAN_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.TIME, Placeholders.VALUE}, new Component[] {getText(plugin.getLocales().getLocaleService().getSystemOrDefaultLocale(), LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), text(user.name()), expire(plugin.getLocales().getLocaleService().getSystemOrDefaultLocale(), ban), TextUtils.replace(getText(plugin.getLocales().getLocaleService().getSystemOrDefaultLocale(), LocalesPaths.COMMANDS_WARN_BAN_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getBan()))}));	
		...
		if(plugin.getMainConfig().getPunishment().getAnnounce().isBan()) {
			if(ban.expirationDate().isPresent()) {
				Sponge.server().onlinePlayers().forEach(player -> {
					player.sendMessage(TextUtils.replaceToComponents(getText(player, LocalesPaths.COMMANDS_BAN_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.TIME, Placeholders.VALUE}, new Component[] {getText(player, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), text(user.name()), expire(player.locale(), ban), TextUtils.replace(getText(player, LocalesPaths.COMMANDS_WARN_BAN_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getBan()))}));
				});
			} else {
				Sponge.server().onlinePlayers().forEach(player -> {
					player.sendMessage(TextUtils.replaceToComponents(getText(player, LocalesPaths.COMMANDS_BAN_ANNOUNCEMENT_PERMANENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {getText(player, LocalesPaths.COMMANDS_WARN_SOURCE_NAME_AUTOPUNISH), text(user.name()), TextUtils.replace(getText(player, LocalesPaths.COMMANDS_WARN_BAN_LIMIT), Placeholders.VALUE, String.valueOf(plugin.getMainConfig().getPunishment().getWarnsBefore().getBan()))}));
				});
			}
