...
public class Kick extends AbstractParameterizedCommand {
	...
	public void execute(CommandContext context, Audience src, Locale locale, boolean isPlayer) throws CommandException {
		...
		if(target.hasPermission(Permissions.IGNORE_BANIP)) exception(TextUtils.replace(getText(locale, LocalesPaths.COMMANDS_KICK_IGNORE), Placeholders.PLAYER, target.name()));
		...
		Sponge.systemSubject().sendMessage(TextUtils.replace(getText(plugin.getLocales().getLocaleService().getDefaultLocale(), LocalesPaths.COMMANDS_KICK_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {source, text(target.name()), reason}));
		if(plugin.getMainConfig().getPunishment().getAnnounce().isKick()) {
			...
			Sponge.server().onlinePlayers().forEach(player -> {
				player.sendMessage(TextUtils.replace(getText(player, LocalesPaths.COMMANDS_KICK_ANNOUNCEMENT), new String[] {Placeholders.SOURCE, Placeholders.PLAYER, Placeholders.VALUE}, new Component[] {source, targetName, reason}));
			});
