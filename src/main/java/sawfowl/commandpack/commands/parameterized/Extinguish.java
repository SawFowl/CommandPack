		...
		if(optTarget.isPresent() && isPlayer && !optTarget.get().uniqueId().equals(((ServerPlayer) src).uniqueId())) {
			...
			if(context.hasPermission(Permissions.EXTINGUISH_STAFF)) {
				...
				if(optEntity.isPresent()) {
					...
					src.sendMessage(TextUtils.replace(getText(locale, LocalesPaths.COMMANDS_EXTINGUISH_SUCCESS_STAFF), Placeholders.PLAYER, ifPlayer ? ((ServerPlayer) target).name() : EntityTypes.registry().valueKey(target.type()).asString()));
					if(ifPlayer) ((ServerPlayer) target).sendMessage(getText(((ServerPlayer) target).locale(), LocalesPaths.COMMANDS_EXTINGUISH_SUCCESS));
					return;
				}
			}
			delay((ServerPlayer) src, locale, consumer -> {
				removeFlame((Entity) src);
				src.sendMessage(getText(locale, LocalesPaths.COMMANDS_EXTINGUISH_SUCCESS));
			});
		}
	}
